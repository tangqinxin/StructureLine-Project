...
int CalibWithButton(std::string strFileName,int m_width,int m_height,float m_square_size) {
    // 路径初始化
    ...
    string FindConnerResultFileName=DirFilePath+"\\"+"findConnerResult.txt";//用于表示是否成功的完全检测到所有的角点

    // 建立一个文件夹保存文件
    MakeMyDirectory(DirFilePath);
    // 标定参数
    ...
    fstream RTMatFile;
    fstream ConnerResultFile;//用于表示是否成功的完全检测到所有的角点
    ...
    ConnerResultFile.open(FindConnerResultFileName, ios::out | ios::trunc);//清空角点检测矩阵
                                                       //关闭文件流
    RMatFile.close();
    ...
    ConnerResultFile.open(FindConnerResultFileName,ios::app);//尾部添加
    ...
    //ConnerResultFile查找角点的输出只要输出0/1即可，因为只需要判断是否找到所有角点，因此不必设置格式
    ...
    while (getline(fin, filename))
    {
        ...
        {
            qDebug() << "can not find chessboard corners!"; //找不到角点
            ConnerResultFile<<imageNumbers<<" "<<"False"<<std::endl;//没有检测到足够的角点
            continue;
        }
        else
        {
            ...
            ConnerResultFile<<imageNumbers<<" "<<"true"<<std::endl;//检测到了足够的角点信息
        }
    }
    ...
    for (int i = 0; i<totalImageNumber; i++)
    {
        ...
        fout << "NO. " << i + 1 << " Image Error:" << singleImageError << " Pixel";
        totalImageError += singleImageError;

        singleImageAverageError = singleImageError / point_counts[i];

        fout << "NO. " << i + 1 << " Image Average Error: " << singleImageAverageError << " Pixel" << endl;
        totalImageAverageError += singleImageAverageError;
    }

    fout << "Total Average Error: " << totalImageError / totalImageNumber << " Pixel";


    fout << "Single Point Error: " << totalImageAverageError / totalImageNumber << " Pixel" << endl << endl;
    ...
    for (int i = 0; i < totalImageNumber; i++)
    {
        fout << "No " << i + 1 << " Rotation Vector: " << endl;
        fout << rotateVectors[i] << endl;
        /* 将旋转向量转换为相对应的旋转矩阵 */
        Rodrigues(rotateVectors[i], rotation_matrix);
        fout << "No " << i + 1 << "Rotation Matrix:" << endl;
        fout << rotation_matrix << endl;
        fout << "No " << i + 1 << "Translation:" << endl;
